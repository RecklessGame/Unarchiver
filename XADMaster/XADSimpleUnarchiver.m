...
-(id)initWithArchiveParser:(XADArchiveParser *)archiveparser entries:(NSArray *)entryarray
{
	if((self=[super init]))
	{
        ...
        if([name matchedByPattern:@"\\.(part[0-9]+\\.rar|tar\\.gz|tar\\.bz2|tar\\.lzma|tar\\.xz|tar\\.Z|sit\\.hqx)$"
                          options:REG_ICASE])
		{
            enclosingdir=[[name stringByDeletingPathExtension] stringByDeletingPathExtension];
		}
		else
		{
...
-(XADError)_unarchiveSubArchive
{
	...
	if(enclosingdir && removesolo)
	{
		if(lookslikesolo)
		{
			...
			finaldestination=[[finalitempath stringByDeletingLastPathComponent] copy];
