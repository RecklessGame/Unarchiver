...
static int start_bunzip(bunzip_data *bd,CSHandle *inhandle,uint32_t *dbuf,bool hasrand);

@implementation XADNSISBzip2Handle
...
-(void)resetBlockStream
{
	...
	start_bunzip(&bd,parent,blocktable,hasrand);
}

-(int)produceBlockAtOffset:(off_t)pos
{
	int actual=read_bunzip(&bd,(char *)outblock,sizeof(outblock));
	if(actual<0) [XADException raiseDecrunchException];
	return actual;
}
...
static unsigned int get_bits(bunzip_data *bd, char bits_wanted)
{
	...
	while (bd->inbufBitCount<bits_wanted) {
		...
		if(bd->inbufPos==bd->inbufCount) {
			if((bd->inbufCount = [bd->inhandle readAtMost:sizeof(bd->inbuf) toBuffer:bd->inbuf]) <= 0)
...
//	if ((i != 0x314159) || (j != 0x265359)) return RETVAL_NOT_BZIP_DATA;
	i = get_bits(bd,8);
	if (i == 0x17) return RETVAL_LAST_BLOCK;
	if (i != 0x31) return RETVAL_NOT_BZIP_DATA;
	if(bd->hasrand) get_bits(bd,1); // skip randomization bit in v1.9x
